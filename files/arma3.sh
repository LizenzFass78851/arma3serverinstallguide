...
steamcmd_interactive_login() {
    ([ -z "$steamcmd_username" ] || [ -z "$steamcmd_password" ]) && echo "Please enter your Steam login credentials."
    [ -z "$steamcmd_username" ] && read -p "Steam Username: " steamcmd_username
    [ -z "$steamcmd_password" ] && read -s -p "Steam Password: " steamcmd_password
    echo
...
createsymlinksformods() {
    cd "$arma3workshop"
    local origmods=$(find "./" -type d -maxdepth 1 | sed 's#^./##g' | sed 's# #%20#g' | sed -e '1d')
...
copybikeys() {
    cd "$arma3workshop"
    local bikeys=$(find "./" | grep "/keys/" | grep ".bikey")
...
install_mods() {
    ...
    steamcmd +force_install_dir $arma3folder +login ${steamcmd_username} ${steamcmd_password} $(for mod_id in "${ids[@]}"; do echo "+workshop_download_item 107410 $mod_id validate"; done) +quit
...
update_all_mods() {
    ...
    cd "$arma3workshop"
    local mod_dirs=$(find . -maxdepth 1 -type d | sed '1d')
    if [ -z "$mod_dirs" ]; then
        echo "No mods found to update."
        return
    fi
    echo "Logging into SteamCMD..."
    steamcmd +force_install_dir $arma3folder +login ${steamcmd_username} ${steamcmd_password} $(for mod_dir in $mod_dirs; do mod_id=$(basename "$mod_dir"); echo "+workshop_download_item 107410 $mod_id validate"; done) +quit
...
update_gameserver() {
    ...
    steamcmd +force_install_dir $arma3folder +login ${steamcmd_username} ${steamcmd_password} +app_update 233780 validate +quit
}
...
print_help() {
    ...
    echo ""
    echo "Options:"
    echo "  --steamcmd-username <username>   Specify the SteamCMD username."
    ...
    echo ""
    echo "Examples:"
    echo "  $0 --steamcmd-username myuser --steamcmd-password mypass --install-mods 123456789,987654321"
    echo "  $0 --steamcmd-username myuser --steamcmd-password mypass --update-all-mods"
    echo "  $0 --steamcmd-username myuser --steamcmd-password mypass --update-gameserver"
}
